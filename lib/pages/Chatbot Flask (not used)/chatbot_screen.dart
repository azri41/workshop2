...
class Chatbot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: ChatbotScreen(title: 'Flutter&Python'),
    );
  }
}
...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  List<String> _data = [];
  static const String BOT_URL = "http://icare-workshop2.herokuapp.com/bot"; // replace with server address
  ...
  void _getResponse(){
    if (_queryController.text.length>0){
      ...
      try{
        client.post(BOT_URL, body: {"msg" : _queryController.text},)
        ..then((response){
          print(response.body);
